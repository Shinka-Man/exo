...
def init_runtime(args):
    ...
    if args.discovery_module == "udp":
        ...
        if not args.discovery_config_path:
            raise ValueError("--discovery-config-path is required when using manual discovery. Please provide a path to a config json file.")
        ...
        raise ValueError(f"Unknown discovery module: {args.discovery_module}")
    ...
    def update_topology_viz(req_id, tokens, __):
        ...
        if not node.inference_engine.shard:
            return
        if node.inference_engine.shard.model_id == 'stable-diffusion-2-1-base':
            return
    ...
    def preemptively_load_shard(request_id: str, opaque_status: str):
        try:
            ...
            if status.get("type") != "node_status" or status.get("status") != "start_process_prompt":
                return
            # Prefer the base shard from the broadcasting node; fall back to its current shard
    ...
    def throttled_broadcast(shard: Shard, event: RepoProgressEvent):
        ...
        if last_event and last_event[1].status == "complete" and event.status == "complete":
            return
        if last_event and last_event[0] == event.status and current_time - last_event[0] < 0.2:
            return
    ...
    return node, api, topology_viz

async def run_model_cli(node: Node, topology_viz, model_name: str, prompt: str):
...
async def main(args):
  ...
  # Initialize runtime now (deferred until inside main)
  node, api, topology_viz = init_runtime(args)
  ...
  if args.command == "run" or args.run_model:
    ...
    if not model_name:
      ...
      return
    await run_model_cli(node, topology_viz, model_name, args.prompt)
    ...
    # Preload disabled to prevent full model loading in memory
    # Each node will load only its assigned shard on first request
    async def _preload_current_shard():
      pass  # Preload temporarily disabled for proper memory distribution
      return

    # asyncio.create_task(_preload_current_shard())  # Commented out
