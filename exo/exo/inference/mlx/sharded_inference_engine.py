...
class MLXDynamicShardInferenceEngine(InferenceEngine):
  ...
  async def infer_tensor(self, request_id: str, shard: Shard, input_data: np.ndarray, inference_state: Optional[dict] = None) -> tuple[np.ndarray, Optional[dict]]:
    ...
    if output_data.dtype == mx.bfloat16:
      output_data = output_data.astype(mx.float32)
      await self._eval_mlx(output_data)
    output_data = await asyncio.get_running_loop().run_in_executor(
      self._mlx_thread,
    ...
    async with self._shard_lock:
      ...
      if self.shard != shard:
        model_shard = await asyncio.get_running_loop().run_in_executor(
          self._mlx_thread,
          # Eagerly load to keep parameters resident and avoid GC unloads
        ...
        # Preload: run a tiny forward pass to warm caches and retain weights
        try:
          def _preload_forward():
            x = mx.array([[1]], dtype=mx.int32)
            _ = self.model(x)
            mx.eval(self.model.parameters())
          await asyncio.get_running_loop().run_in_executor(self._mlx_thread, _preload_forward)
          # Hold a strong reference to prevent GC from unloading parameters
          self.session['model_ref'] = self.model
        except Exception:
          # Preload should not be fatal
          pass
